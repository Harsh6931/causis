  ...
  for (int pass = 0; pass < 8; ++pass) {
    ...
    if (next.instructions.size() == current.instructions.size()) {
      ...
      for (std::size_t i = 0; i < next.instructions.size(); ++i) {
        if (next.instructions[i].opcode != current.instructions[i].opcode ||
            next.instructions[i].int_value != current.instructions[i].int_value ||
            next.instructions[i].bool_value != current.instructions[i].bool_value) {
          same = false;
          break;
        }
      }
...
bool is_const_branch(const std::vector<Instruction>& instructions, std::size_t push_index,
                     bool& condition_value) {
  if (push_index + 1 >= instructions.size()) {
    return false;
  }

  const Instruction& push = instructions[push_index];
  const Instruction& branch = instructions[push_index + 1];
  if (push.opcode != Opcode::PushBool || branch.opcode != Opcode::JumpIfFalse) {
    return false;
  }

  condition_value = push.bool_value;
  return true;
}
...
IrProgram eliminate_dead_branches(const IrProgram& program) {
  ...
  for (std::size_t i = 0; i < instructions.size();) {
    ...
    if (!is_const_branch(instructions, i, condition_value)) {
      output.push_back(instructions[i]);
      ++i;
      continue;
    }
    ...
    if (else_index > i && instructions[else_index - 1].opcode == Opcode::Jump) {
      const std::string& end_label = instructions[else_index - 1].text;
      const auto end_it = labels.find(end_label);
